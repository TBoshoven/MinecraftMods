...
public class MagicDoorknobItem extends Item {
     ...
     */
    MagicDoorknobItem(Properties properties, String typeName, IItemTier tier, ResourceLocation mainTextureLocation, Supplier<Ingredient> ingredient) {
        super(properties);
    ...
    public ActionResultType useOn(ItemUseContext context) {
        ...
        if (!world.isClientSide) {
            ...
            if (canPlaceDoor(world, pos, face, useContext)) {
                ...
                if (doorwayLength > 0) {
                    ItemStack usedItem = context.getItemInHand().copy();
                    usedItem.setCount(1);
                    placeDoor(world, pos, face, usedItem, doorwayLength);
     ...
     */
    private void placeDoor(World world, BlockPos pos, Direction facing, ItemStack doorknob, int doorwayLength) {
        BlockPos doorPos = pos.relative(facing);
        Block block = Objects.requireNonNull(Blocks.MAGIC_DOOR.get());
    ...
    private void placeDoorwayElement(World world, BlockPos pos, boolean isNorthSouth, MagicDoorwayBlock.EnumPartType part) {
        if (isReplaceable(world, pos)) {
            ...
            Block block = Objects.requireNonNull(Blocks.MAGIC_DOORWAY.get());
